...
export const fetchUserProfile = createAsyncThunk<ProfileData, void, { rejectValue: string }>(
  "profile/fetchUserProfile",
  async (_, thunkAPI) => {
    try {
      if (thunkAPI.signal.aborted) return thunkAPI.rejectWithValue("aborted");
      ...
      const safe: ProfileData = {
        ...
        role: d.role ?? "user",
        userImage: d.userImage ?? d.avatar ?? null,
        is_subscription: d.is_subscription ?? null,
      };

      return safe;
    } catch (err: any) {
      if ((axiosClient as any).isCancel?.(err) || err?.message === "thunk aborted by caller") {
        return thunkAPI.rejectWithValue("request_canceled");
      }
...
// kept your cancel handling but tightened typings and variable names.

// no behavior change except more concise error handling.