...
// // <- SafeAreaView & useSafeAreaInsets from safe-area-context (fixes deprecation warning)
...
// import FloatingCoffeeButton from "@/src/components/Button/FloatingCoffeeButton";
// import { SHADOW } from "@/src/assets/utils/shadow";
// import { GlobalShimmerProvider } from "@/src/components/Shimmer/GlobalShimmerProvider";
// import ShopsSkeletonList from "@/src/components/skeltons/ShopsSkeletonList";
// import Shimmer from "@/src/components/Shimmer/Shimmer";
// import CarouselSkeleton from "@/src/components/skeltons/CarouselSkeleton";
// import CommonNewSearchBar from "@/src/Common/CommonNewSearchBar";

// // ---------- NEW imports for cart actions ----------
// import {
//   addToCartAsync,
//   fetchCartAsync,
//   updateCartItemAsync,
...
// type ItemType = { shopname?: ReactNode; menuName?: string; menuId?: number; name?: string; price?: number; imageUrl?: string | null; shopId?: number; menuName?: string; };
// type ShopType = { id: number; shopname: string; shopImage?: string | null; isOpen?: boolean; etaMinutes?: number | null; distanceKm?: number | null; };

// /* ---------------- Constants / Sizes ---------------- */
// const TYPE = { h1: hp(2.8), h2: hp(2.4), body: hp(1.8), small: hp(1.4), micro: hp(1.1), hero: hp(5.2) };
// const PAGE_SIZE = 10;
// const ICON = { small: hp(1.8), medium: hp(2.4), large: hp(3.0) };
// const DAILY_MESSAGES = [
//   "Are you chai? Because you spice up my mornings",
...
// /* ---------------- UI subcomponents (kept memoized) ---------------- */
...
//   const handlePress = () => {
//     scale.value = withTiming(0.94, { duration: 120 });
//     setTimeout(() => (scale.value = withTiming(1, { duration: 240 })), 120);
//     onPress(item);
//   };
...
//   const FONT_SIZE = Math.max(13, Math.round(wp(3.6)));
//   const ITEM_HEIGHT = Math.round(hp(2.2));
//   const tickerItems = useMemo(() => (Array.isArray(DEFAULT_SUGGESTIONS) && DEFAULT_SUGGESTIONS.length ? [...DEFAULT_SUGGESTIONS, DEFAULT_SUGGESTIONS[0]] : []), []);
//   const tickerCount = tickerItems.length;
//   const tickerIndexRef = useRef<number>(0);
//   const tickerIntervalRef = useRef<number | null>(null);
//   const translateY = useSharedValue(0);
...
//   return (
//     <View>
//       <LinearGradient colors={["#5C2C18", "#A65A3A", "#F5E0C3"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={[styles.topHeaderBackground, { paddingTop: (insets.top || (Platform.OS === "android" ? StatusBar.currentHeight ?? 24 : 0)) + hp(2) }]}>
//         <StatusBar translucent backgroundColor="transparent" barStyle="light-content" />
...
//               <View style={{ marginLeft: 10, maxWidth: wp(56) }}>
//                 <Text style={styles.locationTitle} numberOfLines={1}>{location ? "Iskon Junction, Iskcon Ambli..." : "Select Location"}</Text>
//                 <Text style={styles.locationSubtitle} numberOfLines={1}>{location ? `${location.latitude}, ${location.longitude}` : "Tap to choose"}</Text>
//               </View>
//             </TouchableOpacity>
//           </View>
...
//           <Text style={styles.greetingText}>{greeting}, Aarin</Text>
//           <View style={styles.dailyMessageWrapper}>
//             <Text style={styles.dailyMessage} numberOfLines={2} ellipsizeMode="tail" accessibilityLabel={`Daily message: ${dailyMessage}`}>{dailyMessage}</Text>
//           </View>
//         </View>
...
//           inputItemHeight={ITEM_HEIGHT}
//           intervalMs={PAGE_TICKER_INTERVAL}
//           animDuration={420}
//           onNavigate={() => navigation.navigate("searchScreen" as never, { initialQuery: searchText, onDone: (q: string) => setSearchText(q) })}
//           style={{ flex: 1 }}
//           showSpinner={false}
//         />
//       </LinearGradient>
...
//         <CategorySkeleton />
//       ) : (
//         <FlatList data={limitedCategories} horizontal showsHorizontalScrollIndicator={false} keyExtractor={(it: CategoryType) => `cat-${it.categoryId}`} renderItem={({ item }) => <CategoryItem item={item} onPress={onOpenCategory} />} contentContainerStyle={styles.categoryList} />
//       )}
...
//   const nearbyShopsWithin1KM = useMemo(() => {
//     if (!Array.isArray(shops) || shops.length === 0) return [];
//     const withDistance = shops.filter((s) => typeof s.distanceKm === "number" && !isNaN(s.distanceKm));
//     if (withDistance.length === 0) return shops;
//     return withDistance.filter((s) => s.distanceKm! <= 1);
//   }, [shops]);

//   const showNoService = !loadingShops && !isFetching && !!lat && !!lng && nearbyShopsWithin1KM.length === 0;
...
//   const handleOpenCategory = useCallback((c: CategoryType) => {
...
//     // adding new item: require authenticated user and shop id
//     let userId: any = null;
//     try { userId = await getUserIdFromToken(); } catch {}
//     if (!userId) {
//       const raw = await AsyncStorage.getItem("userId");
//       if (raw) userId = raw;
//     }

//     // prefer item.shopId if provided by best-seller payload, otherwise use selectedShopId
...
//       // refresh authoritative cart from server
...
//       setMenuLoading(menuKey, false);
//     }
//   }, [cartMap, dispatch, selectedShopId, isMenuLoading, setMenuLoading]);

//   const handleDecrease = useCallback(async (item: any) => {
//     const menuKey = String(item?.menuId ?? item?.menuName ?? "unknown");
//     if (isMenuLoading(menuKey)) return;
...
//   // derived overlay progress driven by scrollY
//   const overlayProgress = useDerivedValue(() => {
//     const raw = scrollY.value;
//     const t = Math.max(0, Math.min(1, (raw - START) / (END - START)));
//     return withTiming(t, { duration: 220 });
...
//   const statusBarAnimatedStyle = useAnimatedStyle(() => {
//     const bg = interpolateColor(overlayProgress.value, [0, 1], ["transparent", "#ffffff"]);
//     return { backgroundColor: bg, opacity: overlayProgress.value };
//   }, []);

//   const setBarStyle = (style: "light-content" | "dark-content") => {
//     try { StatusBar.setBarStyle(style, true); } catch {}
//   };

//   useAnimatedReaction(() => overlayProgress.value, (curr: number, prev: number | undefined) => {
//     if (prev === undefined) return;
//     if (curr > 0.6 && prev <= 0.6) runOnJS(setBarStyle)("dark-content");
//     else if (curr < 0.4 && prev >= 0.4) runOnJS(setBarStyle)("light-content");
//   }, []);

//   /* ---------------- list helpers & memoized renderers ---------------- */
//   const renderShop = useCallback(({ item }: ListRenderItemInfo<ShopType>) => <ShopCard item={item} onOpen={handleOpenShop} />, [handleOpenShop]);

//   const SHOP_CARD_HEIGHT = Math.round(hp(20)) + 16;
//   const getItemLayout = useCallback((_data: any, idx: number) => ({ length: SHOP_CARD_HEIGHT, offset: SHOP_CARD_HEIGHT * idx, index: idx }), [SHOP_CARD_HEIGHT]);

//   /* ---------------- BestSellerCard component ---------------- */
//   const BestSellerCard = React.memo(({ item }: { item: ItemType }) => {
//     const menuKey = String(item?.menuId ?? item?.menuName ?? "unknown");
//     const qty = cartMap[Number(item.menuId)]?.quantity || 0;
//     const loading = isMenuLoading(menuKey);

//     return (
//       <View style={[styles.popularItem, styles.bestSellerCard]}>
//         <View style={styles.popularCircle}>
//           {item.imageUrl ? <Image source={{ uri: `${apiClient.defaults.baseURL}/uploads/menus/${item.imageUrl}` }} style={styles.popularImg} resizeMode="cover" /> : null}
//         </View>
//         <Text style={styles.popularName} numberOfLines={1}>{item.menuName}</Text>
//         <Text style={{ fontSize: TYPE.small, color: "#666" }}>{item.shopname}</Text>
//         <Text style={styles.popularPrice}>₹{item.price ?? "—"}</Text>

//         {qty > 0 ? (
//           <View style={{ flexDirection: "row", marginTop: hp(0.6), alignItems: "center" }}>
//             <Pressable onPress={() => handleDecrease(item)} disabled={loading} style={{ padding: 6 }}>
//               <Ionicons name="remove-circle-outline" size={hp(3)} color="#6C3F24" />
//             </Pressable>
//             <Text style={{ marginHorizontal: 8, fontWeight: "700" }}>{qty}</Text>
//             <Pressable onPress={() => handleAddOrIncrease(item)} disabled={loading} style={{ padding: 6 }}>
//               <Ionicons name="add-circle-outline" size={hp(3)} color="#6C3F24" />
//             </Pressable>
//           </View>
//         ) : (
//           <TouchableOpacity
//             style={[styles.addToCartBtn, loading ? { opacity: 0.6 } : null]}
//             onPress={() => handleAddOrIncrease(item)}
//             accessibilityRole="button"
//           >
//             <Text style={styles.addToCartText}>+ Add</Text>
//           </TouchableOpacity>
...
//   /* ---------------- Footer with best-sellers (uses BestSellerCard) ---------------- */
//   const FooterComponent = useMemo(() => (
//     <View>
//       <CarouselBanner data={bannerData} />
//       <Text style={[styles.sectionTitle, { paddingHorizontal: wp(5), paddingTop: hp(1) }]}>Best Sellers</Text>
//       {loadingBestSellers ? <PopularSkeleton /> : bestSellers.length > 0 ? (
//         <FlatList
//           data={bestSellers}
//           horizontal
//           showsHorizontalScrollIndicator={false}
//           keyExtractor={(it) => String(it.menuId ?? it.menuName ?? Math.random())}
//           renderItem={({ item }) => <BestSellerCard item={item} />}
//           contentContainerStyle={{ paddingLeft: wp(4), paddingVertical: hp(1) }}
//         />
//       ) : (
//         <View style={{ padding: 20, alignItems: "center" }}><Text style={{ fontSize: 16, color: "#888" }}>No best sellers yet.</Text></View>
//       )}
...
//       </View>
//     </View>
//   ), [loadingBestSellers, bestSellers, cartMap, menuLoadingMap]);
...
//   /* ---------------- IMPORTANT: Reanimated scroll handler (fix for Animated.event error) ----------------
//      We use a proper useAnimatedScrollHandler and pass it into AnimatedFlatList.onScroll
//   */
//   const scrollHandler = useAnimatedScrollHandler((evt) => {
//     // keep scrollY shared value in sync for derived animations
//     scrollY.value = evt.contentOffset?.y ?? 0;
//   });
...
//   return (
//     <SafeAreaView style={styles.safe}>
//       <GlobalShimmerProvider duration={1000}>
//         <StatusBar translucent backgroundColor="transparent" barStyle="light-content" />

//         <Animated.View pointerEvents="none" style={[{
//           position: "absolute", left: 0, right: 0, top: 0,
//           height: insets.top ?? (Platform.OS === "android" ? StatusBar.currentHeight ?? 24 : 0),
//           zIndex: 9999,
//         }, statusBarAnimatedStyle]} />
...
//             keyExtractor={(item: ShopType) => `shop-${item.id}`}
//             renderItem={renderShop}
//             ListHeaderComponent={shops.length > 0 ? HeaderElement : (loadingShops ? (<><Shimmer width={56} height={56} borderRadius={28} style={{ marginLeft: wp(4), marginTop: hp(2) }} /><Shimmer width="60%" height={hp(3)} borderRadius={8} style={{ marginTop: hp(1), marginLeft: wp(4), marginRight: wp(4) }} /><CategorySkeleton /></>) : null)}
//             ListFooterComponent={loadingBestSellers ? (<><CarouselSkeleton /><PopularSkeleton /></>) : FooterComponent}
//             ListEmptyComponent={!loadingShops ? (<View style={styles.noShop}><Ionicons name="location-outline" size={48} color={theme.PRIMARY_COLOR} /><Text style={styles.emptyText}>Sorry, service is not available in your area yet.</Text></View>) : <ShopsSkeletonList count={6} itemHeight={SHOP_CARD_HEIGHT} />}
//             onEndReachedThreshold={0.5}
//             onEndReached={handleLoadMore}
...
//             getItemLayout={getItemLayout}
//             scrollEventThrottle={16}
//             // THIS is the correct Reanimated v2 onScroll usage (no Animated.event)
...
//           entranceDelay={120}
//           loop={true}
//           containerStyle={{ opacity: 0.6, bottom: hp(15), padding: 1, backgroundColor: "white", borderRadius: 50, alignItems: "center", justifyContent: "center" }}
//         />
//       </GlobalShimmerProvider>
//     </SafeAreaView>
//   );
// };
...
// /* ---------------- Styles (kept / minimal adjustments) ---------------- */
// const styles = StyleSheet.create({
//   safe: { flex: 1, backgroundColor: "#fff" },
//   container: { flex: 1, backgroundColor: "#fff" },

//   topHeaderBackground: { paddingBottom: hp(2), overflow: "hidden" },
//   topHeaderInner: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingHorizontal: wp(4) },

//   greetingContainer: { marginTop: hp(2), alignItems: "center" },
//   greetingText: { fontSize: TYPE.h1, fontWeight: "800", color: "#fff", textShadowColor: "rgba(0,0,0,0.18)", textShadowOffset: { width: 0, height: 1 }, textShadowRadius: 3 },
//   dailyMessageWrapper: { marginTop: hp(1), paddingHorizontal: wp(6), width: "100%", alignItems: "center" },

//   dailyMessage: { fontSize: TYPE.small, color: "rgba(255,255,255,0.95)", textAlign: "center", lineHeight: Math.round(TYPE.small * 1.35), maxWidth: wp(86), includeFontPadding: false, fontStyle: "italic", fontWeight: "500" },

//   locationRow: { flexDirection: "row", alignItems: "center", marginLeft: wp(2) },
//   locationBadge: { width: wp(11), height: wp(11), borderRadius: wp(5.5), backgroundColor: "#fff", alignItems: "center", justifyContent: "center" },
//   locationTitle: { fontSize: TYPE.h2, fontWeight: "700", color: "#ffffff" },
//   locationSubtitle: { fontSize: TYPE.small, color: "rgba(255,255,255,0.85)", marginTop: 2 },

//   searchRow: { marginTop: hp(2), marginHorizontal: wp(4), backgroundColor: "white", borderRadius: hp(1.5), flexDirection: "row", alignItems: "center", paddingHorizontal: wp(3), height: hp(6), ...SHADOW },
//   searchIcon: { marginRight: wp(2) },
//   searchInput: { flex: 1, height: "100%", justifyContent: "center" },
//   searchInputText: { fontSize: TYPE.body, color: "#333", paddingVertical: 0 },

//   tickerContainer: { position: "absolute", left: wp(3), right: wp(3), overflow: "hidden", justifyContent: "flex-start" },
//   tickerClip: { overflow: "hidden", justifyContent: "flex-start" },
//   placeholderText: { color: "#999", fontSize: TYPE.body },

//   categoryHeaderRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginTop: hp(1.2), paddingHorizontal: wp(5) },
//   sectionTitle: { fontSize: hp(2), fontWeight: "600", color: "#222" },
//   sectionTitleCompact: { fontSize: hp(2.4), fontWeight: "700", color: "#222", marginTop: hp(1.2), paddingHorizontal: wp(5) },
//   viewAllText: { color: theme.PRIMARY_COLOR, fontWeight: "700", fontSize: TYPE.small },

//   categoryList: { paddingHorizontal: wp(4), paddingVertical: hp(1.2) },
//   popularItem: { width: wp(22), alignItems: "center" },
//   popularCircle: { width: wp(20), height: wp(20), borderRadius: wp(9), backgroundColor: "#fff", justifyContent: "center", alignItems: "center", overflow: "hidden", ...SHADOW },
//   popularImg: { width: "100%", height: "100%", borderRadius: wp(9) },
//   popularName: { fontSize: TYPE.small, color: "#333", textAlign: "center", marginTop: hp(0.6), maxWidth: wp(22) },

//   shopCard: { flexDirection: "row", alignItems: "center", borderBottomWidth: 1, marginHorizontal: wp(5), borderColor: "#eee" },
//   shopImage: { width: wp(20), height: wp(20), borderRadius: 8, backgroundColor: "#f2f2f2", marginTop: hp(0.5) },
//   shopDetails: { flex: 1, padding: wp(3) },
//   shopTitleRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center" },
//   shopName: { fontSize: TYPE.body, fontWeight: "700", color: theme.PRIMARY_COLOR, maxWidth: wp(55) },
//   shopMeta: { flexDirection: "row", alignItems: "center", marginTop: hp(0.5) },
//   metaText: { fontSize: TYPE.small, color: "#666", marginLeft: 6 },
//   tagRow: { flexDirection: "row", flexWrap: "wrap", marginTop: hp(0.6) },
//   tag: { backgroundColor: "#f2f2f2", paddingHorizontal: wp(2), paddingVertical: hp(0.4), borderRadius: 8, marginRight: wp(2), fontSize: TYPE.small, color: "#666" },

//   viewMenuView: { flexDirection: "row", alignItems: "center", gap: wp(2) },
//   statusBadge: { paddingHorizontal: wp(3.0), paddingVertical: hp(0.5), borderRadius: 12 },
//   openBadge: { backgroundColor: "#e6f9ed" },
//   closedBadge: { backgroundColor: "#f0f0f0" },
//   statusText: { fontSize: TYPE.small, color: "#333", fontWeight: "700" },

//   footerHero: { paddingHorizontal: wp(4), paddingTop: hp(2), alignItems: "center", backgroundColor: "#fff" },
//   footerHeroTitle: { fontSize: TYPE.hero, fontWeight: "900", color: "#EDEDED", lineHeight: TYPE.hero * 1.05, textAlign: "center" },
//   footerHeroSubtitle: { fontSize: TYPE.body, color: "#777", marginTop: hp(0.8), textAlign: "center" },

//   noShop: { flex: 1, justifyContent: "center", alignItems: "center", padding: 20 },
//   emptyText: { fontSize: TYPE.body, color: "gray", marginVertical: 10, textAlign: "center" },
...
//   changeLocationText: { color: "white", fontSize: TYPE.small, fontWeight: "700" },
//   comingSoonText: { marginTop: 20, fontSize: TYPE.small, color: "#888", textAlign: "center" },

//   bestSellerCard: { backgroundColor: "#fff", borderRadius: 12, padding: wp(3), marginRight: wp(3), alignItems: "center", width: wp(34), ...SHADOW },
//   addToCartBtn: { marginTop: hp(0.8), paddingVertical: hp(0.6), paddingHorizontal: wp(3), borderRadius: 8, backgroundColor: theme.PRIMARY_COLOR },
//   addToCartText: { color: "#fff", fontSize: TYPE.small, fontWeight: "700" },

//   popularPrice: { fontSize: TYPE.small, fontWeight: "700", marginTop: hp(0.4) },
// });
import React, { useCallback, useEffect, useMemo, useRef, useState, ReactNode } from "react";
import {
  View,
...
import { selectSelectedShopId } from "@/src/Redux/Slice/selectedShopSlice";
import { getUserIdFromToken } from "@/src/assets/utils/getUserIdFromToken";
import AddToCartControl from "@/src/components/Add To Cart Control/AddToCartControl";
import { ROUTES } from "@/src/assets/routes/route";
import { GlobalShimmerProvider } from "@/src/components/Shimmer/GlobalShimmerProvider";
import CommonNewSearchBar from "@/src/Common/CommonNewSearchBar";

// import the shared AddToCartControl component
// -----------------------------------------------------------------
...
const HomeHeader = React.memo(function HomeHeader({
  ...
  return (
    <View>
      <LinearGradient colors={["#5C2C18", "#A65A3A", "#F5E0C3"]}
       start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} 
       style={[styles.topHeaderBackground, { paddingTop: (insets.top || (Platform.OS === "android" ? StatusBar.currentHeight ?? 24 : 0)) + hp(2) ,}]}>
        {/* <StatusBar translucent backgroundColor="transparent" barStyle="light-content" /> */}
...
const HomeScreen: React.FC = () => {
  ...
  const handleOpenCategory = useCallback((c: CategoryType) => {
    navigation.navigate(ROUTES.CATEGORY_DETAIL, { categoryId: c.categoryId, categoryName: c.categoryName });
  }, [navigation]);
  ...
  /* ---------------- BestSellerCard component (now uses shared AddToCartControl) ---------------- */
  const BestSellerCard = React.memo(({ item }: { item: ItemType }) => {
    return (
      <View style={[styles.popularItem, styles.bestSellerCard]}>
        <View style={styles.popularCircle}>
        ...
        {/* shared control handles optimistic UI + animations */}
        <AddToCartControl item={item} style={{ marginTop: hp(0.6) }} />
      </View>
    );
  });
  BestSellerCard.displayName = "BestSellerCard";
