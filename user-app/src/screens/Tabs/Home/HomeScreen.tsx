...
//     return {
//       data,
//       nextOffset: data.length === 0 ? null : offset + PAGE_SIZE,
//       total,
//     };
//   };
//   // --- Add below existing API wrappers ---
...
//     retry: 1,
//   });
// // near top of your HomeScreen component (with other callbacks)
// const handleOpenCategory = useCallback(
//   (c: CategoryType) => {
//     // NOTE: CategoryDetailScreen uses route param names: categoryId, categoryName
//     // The navigator route name used in CategoryDetailScreen file is "CategoryDetail".
//     // If your navigator registers the screen under a different name, change it here.
...
//       () => (shopPages ? shopPages.pages[0]?.total ?? null : null),
//       [shopPages]
//     );
...
//         hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}
//         accessibilityRole="button"
//         accessibilityLabel={`Open ${item.categoryName} category`}
//       >
...
//             <Text style={styles.sectionTitle}>Best Sellers</Text>
  ...
  //           {item.imageUrl ? (
  //             <Image
  //               source={{ uri: `${apiClient.defaults.baseURL}/uploads/menus/${item.imageUrl}` }}
  //               style={styles.popularImg}
  //               resizeMode="cover"
  //             />
  //           ) : null}
      ...
      {dailyMessage}
        ...
        <View style={styles.searchRow}>
          ...
          >
            ...
            {!searchText && (
              <View style={[styles.tickerContainer, { height: ITEM_HEIGHT }]}>
                <Animated.View style={[animatedStyle]}>
                  {DEFAULT_SUGGESTIONS.map((w, i) => (
                    <Text key={i} style={[styles.placeholderText, { height: ITEM_HEIGHT, lineHeight: ITEM_HEIGHT }]}>
                      Search for {w}
                    </Text>
                  ))}
                </Animated.View>
  ...
  return (
    ...
    <View style={{ padding: 20, alignItems: "center" }}>
      <Text style={{ fontSize: 16, color: "#888" }}>No best sellers yet.</Text>
          ...
          onEndReached={handleLoadMore}
          refreshControl={
            <RefreshControl refreshing={isFetching} onRefresh={onRefresh} tintColor={theme.PRIMARY_COLOR} />
          }
          contentContainerStyle={{ paddingBottom: hp(10) }}
          initialNumToRender={6}
          maxToRenderPerBatch={8}
