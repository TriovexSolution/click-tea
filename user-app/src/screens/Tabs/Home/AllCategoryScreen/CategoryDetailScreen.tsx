...
// import NewCommonSearchBar from "@/src/Common/NewCommonSearchBar";
// import CommonUI from "@/src/Common/NewCommonSearchBar";
...
//               style={[
//                 styles.addButton,
//                 { backgroundColor: "#ccc", minWidth: wp(16) },
//               ]}
//             >
//               <Text style={[styles.addBtnText, { color: "#666" }]}>
//                 Unavailable
//               </Text>
//             </View>
//           )
...
//   if ((a.cartEntry?.quantity || 0) !== (b.cartEntry?.quantity || 0))
//     return false;
...
//     if (!err) return false;
//     if (
//       typeof axiosClient?.isCancel === "function" &&
//       axiosClient.isCancel(err)
//     )
//       return true;
...
//           opts && Object.prototype.hasOwnProperty.call(opts, "search");
...
//         if (searchToUse != null && String(searchToUse).trim().length > 0)
//           params.search = String(searchToUse).trim();
...
//             cur.forEach((m) =>
//               map.set(m.menuId ?? m.menuName ?? JSON.stringify(m), m)
//             );
//             dataMenus.forEach((m) =>
//               map.set(m.menuId ?? m.menuName ?? JSON.stringify(m), m)
//             );
...
//         Alert.alert(
//           "Shop not selected",
//           "Please select a shop before adding items."
//         );
//         return;
//       }
...
//       {/* Search area (now using CommonSearchBar) */}
//       <View style={{ marginHorizontal: wp(5), marginTop: hp(2) }}>
//         <CommonUI.NewCommonSearchBar
//           value={query}
//           onChange={(v) => setQuery(v)}
//           onSubmit={(v?: string) => {
//             const term = (v ?? query ?? "").trim();
//             if (!term) {
//               // if empty search submitted, clear and load base list
//               handleClear();
//               return;
//             }
...
//               debounceRef.current = null;
//             }
//             // update both query and debouncedQuery to avoid race with debounce timer
//             setQuery(term);
//             setDebouncedQuery(term);
//             // abort any inflight request and fetch immediately with explicit search override
//             safeAbort();
//             fetchMenus({ page: 1, replace: true, search: term });
//             Keyboard.dismiss();
//           }}
//           onClear={handleClear}
//           placeholder={`Search ${categoryName ?? "items"}`}
//           loading={isSearching || loading}
//           // no suggestions passed → ticker disabled. If you want ticker, pass suggestions={['tea','coffee']}
//           suggestions={[]}
//         />
//       </View>
...
//               <TouchableOpacity
//                 onPress={() => fetchMenus({ page: 1, replace: true })}
...
// Redux
import { useDispatch, useSelector, shallowEqual } from "react-redux";
import { addToCartAsync, fetchCartAsync, selectCartItems, updateCartItemAsync } from "@/src/Redux/Slice/cartSlice";
import { selectSelectedShopId } from "@/src/Redux/Slice/selectedShopSlice";
import CartIconWithBadge from "@/src/components/CartIconBadge";
import { getUserIdFromToken } from "@/src/assets/utils/getUserIdFromToken";
import axiosClient from "@/src/api/client";
...
import MenuListSkeleton from "@/src/components/skeltons/MenuListSkeleton";
...
const DEFAULT_LIMIT = 10;
const MAX_LIMIT = 100;
const MIN_LOAD_MORE_DELTA = 800; // ms between subsequent onEndReached triggers
...
const MenuRow = React.memo(
  function MenuRow({
    ...
    const [imgFailed, setImgFailed] = useState(false);
    const imgSource =
      !imgFailed && item.imageUrl ? { uri: `${BASE_URL}/uploads/menus/${item.imageUrl}` } : require("@/src/assets/images/onBoard1.png");
    ...
    return (
      <View style={styles.menuRow} accessible accessibilityRole="button">
        ...
        <View style={styles.menuRight}>
          {item.rating && item.rating > 0 ? (
            <View style={styles.popularBadgeAbsolute} accessible accessibilityLabel="Popular">
              <Text style={styles.popularText}>Popular</Text>
            </View>
          ) : null}

          <View style={styles.addControlWrap}>
            {/* pass disabled and onSuccess so parent can keep per-item loading / analytics */}
            <AddToCartControl item={item} disabled={disabled} onSuccess={onSuccess} />
          </View>
        </View>
      </View>
    );
  ...
  (prev, next) => {
    ...
    if (prev.item?.menuId !== next.item?.menuId) return false;
    if (prev.item?.menuName !== next.item?.menuName) return false;
    if ((prev.item?.imageUrl || "") !== (next.item?.imageUrl || "")) return false;
    if ((prev.qty || 0) !== (next.qty || 0)) return false;
...
const CategoryDetailScreen = ({ route }: any) => {
  ...
  // per-item loading map + cheap version counter for extraData
  const [menuLoadingMap, setMenuLoadingMap] = useState<Record<string, boolean>>({});
  const [menuLoadingVersion, setMenuLoadingVersion] = useState(0);
  const setMenuLoading = useCallback((id: string, v = true) => {
    setMenuLoadingMap((prev) => {
      if (prev[id] === v) return prev;
      const next = { ...prev, [id]: v };
      // bump version (cheap scalar) to trigger FlatList extraData updates
      setMenuLoadingVersion((s) => s + 1);
      return next;
    });
  }, []);
  ...
  // simple throttle for loadMore
  const lastLoadMoreTs = useRef(0);
  const loadMore = useCallback(() => {
    if (loadingMore || loading) return;
    if (typeof total === "number" && menus.length >= total) return;
    const now = Date.now();
    if (now - lastLoadMoreTs.current < MIN_LOAD_MORE_DELTA) return;
    lastLoadMoreTs.current = now;
    fetchMenus({ page: page + 1 });
  }, [loadingMore, loading, total, menus.length, fetchMenus, page]);

  /* ---------------- Cart actions (kept in parent to support menuLoadingMap) ---------------- */
  const handleAddOrIncrease = useCallback(
    async (item: MenuItem) => {
      const menuIdKey = String(item?.menuId ?? item?.menuName ?? "unknown");
      if (isMenuLoading(menuIdKey)) return;
  ...
  const keyExtractor = useCallback(
    (item: MenuItem, index: number) => {
      const idPart = item?.menuId != null ? String(item.menuId) : `${item.menuName ?? "item"}-${index}`;
      return `${categoryIdStr}-${idPart}`;
    },
    [categoryIdStr]
  );
  ...
  const onAddSuccess = useCallback(
    (menu: MenuItem) => {
      // Called by AddToCartControl when it successfully changes cart.
      // We refresh cart to keep canonical state.
      (async () => {
        try {
          if (shopId) await dispatch(fetchCartAsync({ shopId })).unwrap();
        } catch {}
  ...
  const renderItem = useCallback(
    ({ item }: { item: MenuItem }) => {
      ...
      // We still keep parent handlers in case AddToCartControl doesn't perform the action
      // or to allow parent to orchestrate loading. We pass disabled + onSuccess for cooperation.
      return (
        <MenuRow
          item={item}
          qty={qty}
    ...
    [cartMap, isMenuLoading, onAddSuccess]
  );
  ...
  return (
    <SafeAreaProvider style={styles.container}>
      ...
      <View style={styles.searchContainer}>
        ...
        {isSearching ? <ActivityIndicator size="small" color={theme.PRIMARY_COLOR} style={{ marginLeft: 8 }} /> : null}
      ...
      {loading && menus.length === 0 ? (
        ...
        <>
          ...
          <FlatList
            ...
            extraData={menuLoadingVersion} // cheap scalar; cartMap is derived from selector so component re-renders when cart changes
          />
        </>
      )}
...
const styles = StyleSheet.create({
  ...
  // RIGHT SIDE: popular badge pinned top-right, Add control centered vertically
  menuRight: {
    width: wp(28),
    alignItems: "flex-end",
    justifyContent: "center",
  ...
  // absolute-style popular badge (keeps top-right position)
  popularBadgeAbsolute: {
    position: "absolute",
    top: hp(0.6),
    right: 0,
    backgroundColor: "#EAF9F1",
