...
// const normalizeStatus = (s?: string) => (s || "preparing").trim();
// const labelize = (s: string) =>
//   s ? s.charAt(0).toUpperCase() + s.slice(1) : "";

// const OrderCard = React.memo(
//   ({ item, ongoing, onCancel, onView }: any) => {
//     const status = normalizeStatus(item.status);
...
//               onPress={() => onCancel(item.orderId)}
//             >
//               <Text style={[styles.btnText, { color: "#F33" }]}>
//                 Cancel
//               </Text>
//             </TouchableOpacity>
//           )}
...
//   const goToDetails = useCallback(
//     (orderId: number) =>
//       navigation.navigate("orderDetailScreen" as never, { orderId } as never),
//     [navigation]
//   );
...
import OrdersSkeleton from "@/src/components/skeltons/OrdersSkeleton";
...
const OrderCard = React.memo(
  ({ item, ongoing, onCancel, onView, cancelingId }: any) => {
    const status = normalizeStatus(item.status);
    const isCancelling = cancelingId === item.orderId;
        ...
        <View style={styles.buttonRow}>
          {ongoing && (
            <TouchableOpacity
              style={[
                styles.actionBtn,
                { borderColor: "#F66", opacity: isCancelling ? 0.6 : 1 },
              ]}
              onPress={() => onCancel(item.orderId)}
              disabled={isCancelling}
            >
              {isCancelling ? (
                <ActivityIndicator size="small" color="#F33" />
              ) : (
                <Text style={[styles.btnText, { color: "#F33" }]}>Cancel</Text>
              )}
            </TouchableOpacity>
...
const OrdersScreen = () => {
  ...
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [cancelingId, setCancelingId] = useState<number | null>(null); // <-- track per-order cancel state
  ...
  // confirm dialog shown before cancelling
  const confirmCancel = useCallback(
    (orderId: number) => {
      Alert.alert(
        "Cancel Order",
        "If you cancel this order you will not get a cash refund. If the order was paid using ClickTea Coins you will receive coins back to your balance. Do you want to continue?",
        [
    ...
    [] // cancelOrder defined below but we reference it within same component; React will use latest via closure
  );
  ...
  const cancelOrder = useCallback(
    async (orderId: number) => {
      ...
      try {
        ...
        // server response might include refund info when payment_type === 'coin'
        const data = res?.data;
        ...
        Alert.alert("Cancelled", successMessage);
      } catch (err: any) {
        console.error("❌ Cancel error:", err?.response?.data || err);
        const serverMsg = err?.response?.data?.message;
        Alert.alert("Cancel failed", serverMsg || "Could not cancel order. Try again.");
      } finally {
        setCancelingId(null);
      }
    },
  ...
  const goToDetails = useCallback(
    (orderId: number) =>
      navigation.navigate(ROUTES.ORDER_DETAILS as never, { orderId } as never),
    [navigation]
  );
  ...
  return (
    <SafeAreaContainer>
      <StatusBar
          ...
          translucent // false ensures the content is below status bar
        />
      <CommonHeader title="Order History" />
        ...
        <SectionList
          ...
          renderItem={({ item }) => (
            <OrderCard
              item={item}
              ongoing={item._status === "preparing"}
              onCancel={confirmCancel} // show confirm first
              onView={goToDetails}
