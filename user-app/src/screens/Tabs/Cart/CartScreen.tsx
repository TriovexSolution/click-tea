...
// const groupByShop = (items: cartDataType[]) => {
//   return items.reduce<
//     Record<number, { shopId: number; items: cartDataType[] }>
//   >((acc, it) => {
//     const sid = Number((it as any).shopId ?? 0);
//     if (!acc[sid]) acc[sid] = { shopId: sid, items: [] };
//     acc[sid].items.push(it);
...
//   const [deliveryNoteLocal, setDeliveryNoteLocal] = useState<string>("");
//   const [savedDeliveryNote, setSavedDeliveryNote] = useState<string>("");

//   const { selectedAddress } = useAddress();
//   const mountedRef = useRef(true);
//   const fetchAbortRef = useRef<AbortController | null>(null);
//   const bottomSheetRef = useRef<BottomSheetModal | null>(null);
//   const snapPoints = useMemo(() => [hp(45), hp(80)], []);
//   const [randomMsg, setRandomMsg] = useState("");
//   const tapTimeoutsRef = useRef<Record<number, number | null>>({});
//   const queuedDeltasRef = useRef<Record<number, number>>({});
...
//         const currentItem = cartDataRef.current.find(
//           (c) => c.cartId === cartId
//         );
//         const desiredQty = currentItem
//           ? currentItem.quantity ?? Math.max(0, oldQty + totalDelta)
...
//         try {
//           await dispatch(
//             updateCartItemAsync({ cartId, quantity: desiredQty })
//           ).unwrap();
//           dispatch(fetchCartAllAsync()).catch(() => {});
//         } catch (err) {
//           console.error("Batched update failed", err);
...
//   const paymentAbortRef = useRef<AbortController | null>(null);
//   const [processingPaymentLocal, setProcessingPaymentLocal] = useState(false);

//   const handleCoinPayment = useCallback(async () => {
//     if (processingPaymentLocal) return;
//     if (!cartData.length) return showToast("Cart is empty");

//     if (useCoins && (coinBalance ?? 0) < totalAmount) {
//       Alert.alert(
//         "Insufficient Coins",
//         `You have ₹${coinBalance}, needed ₹${totalAmount}`
//       );
//       return;
//     }

//     setProcessingPaymentLocal(true);
//     paymentAbortRef.current?.abort();
//     const controller = new AbortController();
//     paymentAbortRef.current = controller;
...
//         await api.post("/api/coin/pay", shopPayload, {
//           signal: controller.signal as any,
//         });
//       }
...
//       }
//     } finally {
//       if (mountedRef.current) setProcessingPaymentLocal(false);
//     }
//   }, [
//     cartData,
//     totalAmount,
...
//       const res = await api.post("/api/orders/pay-later", {
//         cartItems,
//         delivery_note: deliveryNoteLocal || "",
//       });
...
//     (type: "toPay" | "proceed" | "deliveryInstruction") => {
//       setModalType(type);
//       // show sheet then expand or keep default depending on type
//       bottomSheetRef.current?.present?.();
//       // if delivery instruction, expand right away so keyboard has space
//       if (type === "deliveryInstruction") {
//         // slight delay to ensure present() finished
//         setTimeout(() => bottomSheetRef.current?.expand?.(), 120);
//       }
//       stopFloat();
//     },
//     [stopFloat]
...
//     [useCoins, totalAmount, coinBalance]
//   );
//   const amountAfterCoins = useMemo(
//     () => totalAmount - coinsToUse,
//     [totalAmount, coinsToUse]
//   );
//   const gstAmount = useMemo(
//     () => Number((amountAfterCoins * GST_RATE).toFixed(2)),
//     [amountAfterCoins]
//   );
//   const finalAmount = useMemo(
//     () => Number((amountAfterCoins + gstAmount).toFixed(2)),
...
//   const handleSaveDeliveryNote = useCallback(() => {
//     // Save locally for UI preview; in production you'd also persist to server if needed
//     setSavedDeliveryNote(deliveryNoteLocal.trim());
//     showToast("Delivery note saved");
//     bottomSheetRef.current?.dismiss?.();
//     Keyboard.dismiss();
//   }, [deliveryNoteLocal]);
...
//             totalAmount: Number(
//               cartItemsPayload
//                 .reduce((s, x) => s + (x.price || 0) * (x.quantity || 0), 0)
//                 .toFixed(2)
//             ),
//             payment_type:
//               paymentType === "ONLINE"
...
//   // Display row text: saved delivery note or default hint
//   const deliveryRowText = useMemo(() => {
//     if (savedDeliveryNote && savedDeliveryNote.trim().length > 0)
//       return savedDeliveryNote;
//     return "Tap to add or edit delivery note";
//   }, [savedDeliveryNote]);

//   return (
//     <SafeAreaProvider style={[styles.container, { paddingBottom: insets.bottom,paddingTop:insets.top   }]}>
//       <CommonHeader title="Cart" />
...
//           >
//             <View
//               style={{ flexDirection: "column", flex: 1, paddingRight: wp(2) }}
//             >
//               <Text style={styles.deliveryInformationText}>
//                 Delivery Instruction
//               </Text>
...
//                   { color: savedDeliveryNote ? "#333" : "#AAAAAA" },
//                 ]}
//                 numberOfLines={2}
//               >
//                 {deliveryRowText}
...
//         enablePanDownToClose
//         handleIndicatorStyle={{ backgroundColor: "#ddd" }}
//         // keep removeClippedSubviews false to avoid odd resizing with keyboard
//       >
//         <BottomSheetView style={styles.bottomSheetView}>
//           {modalType === "toPay" ? (
//             <View>
...
//             </View>
//           ) : modalType === "deliveryInstruction" ? (
//             // KeyboardAvoidingView + explicit expand on focus -> prevents weird stretching when keyboard opens
//             <KeyboardAvoidingView
...
//                   }}
//                   onFocus={() => {
//                     // expand sheet to full snap so keyboard has enough space
//                     try {
//                       bottomSheetRef.current?.expand?.();
//                     } catch {}
//                   }}
//                   onBlur={() => {
//                     // collapse back to default snap (makes appearance predictable)
//                     try {
...
import { Ionicons } from "@expo/vector-icons";
import { useFocusEffect, useNavigation } from "@react-navigation/native";
import { hp, wp } from "@/src/assets/utils/responsive"; // keeps your responsive helpers centralized
import { useSelector, useDispatch, shallowEqual } from "react-redux";
import { selectSelectedShopId } from "@/src/Redux/Slice/selectedShopSlice";
import type { cartDataType } from "@/src/assets/types/userDataType";
import axios, { AxiosResponse } from "axios";
...
// ---------- NOTES ----------
// - This is an optimized + commented version of your CartScreen.
// - I removed the `Alert.alert` success popup on PayLater and replaced it with showToast.
// - Responsiveness: you already use hp/wp utilities — I've added comments where to tweak.
// - Performance: heavy handlers memoized with useCallback/useMemo, minimized re-renders.
// ---------------------------
...
const showToast = (msg: string) => {
  if (Platform.OS === "android") ToastAndroid.show(msg, ToastAndroid.SHORT);
  else console.info("Toast:", msg); // fallback for iOS during dev (you can swap to a Toast library)
};

// Keep CartItemRow as memoized component
const CartItemRow = React.memo(function CartItemRow({
  item,
  isUpdating = false,
  onQtyChange,
  ...
  const triggerBounce = () => {
    qtyScale.value = 1.2;
    qtyScale.value = withTiming(1, { duration: 260, easing: Easing.out(Easing.quad) });
    priceScale.value = 1.15;
    priceScale.value = withTiming(1, { duration: 260, easing: Easing.out(Easing.quad) });
  };
  ...
  return (
    <Animated.View style={[styles.itemRow, shakeStyle]}>
      ...
      <Animated.Text style={[styles.itemTotal, priceStyle]}>₹{subtotal.toFixed(2)}</Animated.Text>
...
const CartScreen: React.FC = () => {
  ...
  const dispatch = useDispatch<any>();
  const shopId = useSelector(selectSelectedShopId);
  const coinBalance = useSelector((s: RootState) => s.profile.data?.coinBalance ?? 0, shallowEqual);
  const isSubscribed = useSelector((s: RootState) => s.profile.data?.is_subscription === 1);
  ...
  const [useCoins, setUseCoins] = useState(false);
  const [showFooterUI, setShowFooterUI] = useState(false);
  const [modalType, setModalType] = useState<"toPay" | "proceed" | "deliveryInstruction" | null>(null);
  const [processingPayment, setProcessingPayment] = useState(false);
  const [deliveryNoteLocal, setDeliveryNoteLocal] = useState<string>("");
  ...
  useEffect(() => {
    ...
    return () => {
      mountedRef.current = false;
      fetchAbortRef.current?.abort();
      Object.values(tapTimeoutsRef.current).forEach((t) => t && clearTimeout(t as number));
      tapTimeoutsRef.current = {};
      queuedDeltasRef.current = {};
    };
  }, []);
  ...
  const fetchLocalCart = useCallback(async () => {
    ...
    try {
      const res: AxiosResponse = await api.get(`/api/cart`, { signal: controller.signal as any });
      const payload = Array.isArray(res.data) ? res.data : [];
  ...
  useFocusEffect(useCallback(() => { fetchLocalCart(); }, [fetchLocalCart]));

  const handleRemoveRequested = useCallback(async (cartId: number) => {
    // clear local queued updates for this cartId
    if (tapTimeoutsRef.current[cartId]) { clearTimeout(tapTimeoutsRef.current[cartId] as number); delete tapTimeoutsRef.current[cartId]; }
    if (queuedDeltasRef.current[cartId]) delete queuedDeltasRef.current[cartId];
  ...
  const totalAmount = useMemo(() => cartData.reduce((sum, it) => {
    const unit = it.snapshotPrice != null ? Number(it.snapshotPrice) : Number(it.price ?? 0);
    return sum + unit * (it.quantity || 0);
  }, 0), [cartData]);

  const totalItems = useMemo(() => cartData.reduce((s, it) => s + (it.quantity || 0), 0), [cartData]);

  const updateLocalQty = useCallback((cartId: number, newQty: number) => {
    setCartData((prev) => prev.map((i) => (i.cartId === cartId ? { ...i, quantity: newQty } : i)));
  }, []);

  const revertLocalQty = useCallback((cartId: number, oldQty: number) => {
    setCartData((prev) => prev.map((i) => (i.cartId === cartId ? { ...i, quantity: oldQty } : i)));
  }, []);

  const BATCH_DELAY = 450; // ms

  const handleQtyChange = useCallback((item: cartDataType, increment: boolean) => {
    if (!item || !item.cartId) return;
    const cartId = item.cartId;
    const oldQty = item.quantity ?? 0;
    const delta = increment ? 1 : -1;
    ...
    if (newQty <= 0) {
      // instead of alert, use JS confirm or a proper modal — keeping simple here
      // Ask before removing (use native Alert if you want). We keep current UX.
      handleRemoveRequested(cartId);
      return;
    }

    updateLocalQty(cartId, newQty);
    queuedDeltasRef.current[cartId] = (queuedDeltasRef.current[cartId] || 0) + delta;
    setItemUpdating(cartId, true);
    ...
    tapTimeoutsRef.current[cartId] = setTimeout(async () => {
      ...
      const currentItem = cartDataRef.current.find((c) => c.cartId === cartId);
      const desiredQty = currentItem ? currentItem.quantity ?? Math.max(0, oldQty + totalDelta) : Math.max(0, oldQty + totalDelta);
  ...
  }, [dispatch, revertLocalQty, updateLocalQty, handleRemoveRequested]);
  ...
  const handleCoinPayment = useCallback(async () => {
    ...
    if (useCoins && (coinBalance ?? 0) < totalAmount) {
      showToast(`You have ₹${coinBalance}, needed ₹${totalAmount.toFixed(2)}`);
      return;
    }
    ...
    try {
      ...
      for (const sidStr of Object.keys(groups)) {
        ...
        const shopIdForReq = g.shopId;
        const shopPayload = {
          totalAmount: Number(g.items.reduce((s, it) => {
            const price = typeof it.snapshotPrice === "number" ? it.snapshotPrice : Number(it.price ?? 0);
            return s + price * (it.quantity || 0);
          }, 0).toFixed(2)),
          shopId: shopIdForReq,
      ...
      // success -> clear local and re-sync
      setCartData([]);
      dispatch(resetCart());
      dispatch(fetchCartAllAsync()).catch(() => { });
      showToast("Order(s) placed using coins!");
  ...
  // NOTE: PayLater no longer shows Alert.alert on success — we use showToast and navigation
  const handlePayLater = useCallback(async () => {
    if (processingPayment) return;
    if (!cartData.length) return showToast("Cart is empty");
    setProcessingPayment(true);
    ...
    try {
      const cartItems = cartData.map((it) => ({
        ...
        variantId: it.variantId ?? null,
        quantity: it.quantity,
        unitPrice: typeof it.snapshotPrice === "number" ? it.snapshotPrice : Number(it.price ?? 0),
        addons: it.addons || [],
        shopId: it.shopId,
        cartId: it.cartId // send cartId so server can delete exact rows
      }));
      ...
      // removed Alert.alert, replaced with toast + navigation
      showToast(`Pay Later order placed. ₹${res.data.totalAmount}`);
  ...
  const keyExtractor = useCallback((it: cartDataType) => `${it.cartId ?? it.menuId}`, []);

  const renderItem = useCallback(({ item }: { item: cartDataType | null }) => {
    if (!item) return null;
    const isUpdating = !!(item.cartId && updatingMap[item.cartId]);
    return <CartItemRow item={item} isUpdating={isUpdating} onQtyChange={handleQtyChange} onRemoveRequested={handleRemoveRequested} />;
  }, [handleQtyChange, handleRemoveRequested, updatingMap]);

  const getItemLayout = useCallback((_data: any, index: number) => ({ length: APPROX_ITEM_HEIGHT, offset: APPROX_ITEM_HEIGHT * index, index }), []);

  useEffect(() => { const msg = messages[Math.floor(Math.random() * messages.length)]; setRandomMsg(msg); }, []);

  // Floating animation control (kept same but memoized handlers)
  const FLOAT_AMPLITUDE = hp(1.2);
  const floatY = useSharedValue(0);
  const stopFloatTimer = useRef<number | null>(null);

  const startFloat = useCallback(() => {
    if (stopFloatTimer.current) { clearTimeout(stopFloatTimer.current); stopFloatTimer.current = null; }

    floatY.value = withRepeat(withSequence(withTiming(-FLOAT_AMPLITUDE, { duration: 800, easing: Easing.inOut(Easing.quad) }), withTiming(0, { duration: 800, easing: Easing.inOut(Easing.quad) })), -1, false);

    stopFloatTimer.current = setTimeout(() => { try { cancelAnimation(floatY); } catch (e) { } floatY.value = 0; stopFloatTimer.current = null; }, 5000) as unknown as number;
  }, [floatY]);

  const stopFloat = useCallback(() => { try { cancelAnimation(floatY); } catch (e) { } floatY.value = 0; if (stopFloatTimer.current) { clearTimeout(stopFloatTimer.current); stopFloatTimer.current = null; } }, [floatY]);

  useEffect(() => { if (cartData.length > 0) startFloat(); else stopFloat(); }, [cartData.length, startFloat, stopFloat]);
  useEffect(() => { if (showFooterUI) stopFloat(); }, [showFooterUI, stopFloat]);

  const floatStyle = useAnimatedStyle(() => ({ transform: [{ translateY: floatY.value }] }));

  const openPaymentModal = useCallback((type: "toPay" | "proceed" | "deliveryInstruction") => { setModalType(type); bottomSheetRef.current?.present?.(); stopFloat(); }, [stopFloat]);
  const onPressFloating = useCallback(() => { setShowFooterUI(true); startFloat(); }, [startFloat]);

  const gradientColors = useMemo(() => ["#562E19", "#943400", "#D97706"], []);
  const insets = useSafeAreaInsets();

  // coins math
  const coinsToUse = useMemo(() => (useCoins ? Math.min(totalAmount, coinBalance ?? 0) : 0), [useCoins, totalAmount, coinBalance]);
  const amountAfterCoins = useMemo(() => totalAmount - coinsToUse, [totalAmount, coinsToUse]);
  const gstAmount = useMemo(() => Number((amountAfterCoins * GST_RATE).toFixed(2)), [amountAfterCoins]);
  const finalAmount = useMemo(() => Number((amountAfterCoins + gstAmount).toFixed(2)), [amountAfterCoins, gstAmount]);
  const handleSaveDeliveryNote = useCallback(() => { showToast("Delivery note saved"); bottomSheetRef.current?.dismiss?.(); }, []);

  const groupsMemo = useMemo(() => groupByShop(cartData), [cartData]);
  ...
  return (
    <SafeAreaView style={[styles.container, { paddingBottom: insets.bottom }]}>
      ...
      {loading ? (
        <View style={styles.emptyCart}><ActivityIndicator size="large" color={theme.PRIMARY_COLOR} /></View>
      ) : cartData.length === 0 ? (
        <View style={styles.emptyCart}>
          <View style={styles.emptyTextWrapper}><Text style={styles.emptyTitle}>{randomMsg}</Text></View>
          <Pressable onPress={() => navigation.navigate("Home")} accessibilityRole="button" style={({ pressed }) => [styles.button, pressed && { opacity: 0.92 }]}>
            <Text style={styles.buttonText}>Go to Shop</Text>
          </Pressable>
        </View>
      ) : (
        <View style={{ flex: 1 }}>
          <View style={styles.addressRow}>
            <Pressable style={{ flexDirection: "row", alignItems: "center", flex: 1 }} onPress={() => navigation.navigate("changeAddressScreen")}>
              <Ionicons name="location-outline" size={hp(3.5)} />
              <View style={{ marginLeft: wp(2), flex: 1 }}>
                {selectedAddress ? (
                  <View>
                    <Text style={{ fontWeight: "600" }}>{String(selectedAddress.addressType ?? "")}</Text>
                    <Text style={styles.addressSub}>{String(selectedAddress.houseNumber ?? "")}{selectedAddress.houseNumber ? ", " : ""}{String(selectedAddress.roadArea ?? "")}</Text>
                  </View>
                ) : (
                  <View>
                    <Text style={{ fontWeight: "600" }}>Select Delivery Address</Text>
                    <Text style={styles.addressSub}>Tap to choose or add an address</Text>
                  </View>
                )}
              </View>
            </Pressable>

            <Pressable style={styles.changeBtn} onPress={() => navigation.navigate("changeAddressScreen")}>
              <Text style={styles.changeBtnText}>Change</Text>
            </Pressable>
          </View>
          ...
          <View style={styles.itemsHeader}>
            <Text>{`Items (${totalItems})`}</Text>
            <View style={styles.deliveryInfo}><Ionicons name="time-outline" size={hp(3)} /><Text style={styles.deliveryText}>Delivery in 10 mins</Text></View>
          </View>

          <FlatList data={cartData} renderItem={renderItem} keyExtractor={keyExtractor} contentContainerStyle={{ paddingBottom: showFooterUI ? hp(40) + insets.bottom : hp(24) + insets.bottom }} initialNumToRender={6} maxToRenderPerBatch={8} windowSize={9} removeClippedSubviews getItemLayout={getItemLayout} />
        </View>
      )}

      {cartData.length > 0 && !showFooterUI && (
        <Animated.View style={[styles.floatingWrapper, floatStyle]}>
          <Pressable onPress={onPressFloating} accessibilityRole="button" style={({ pressed }) => [styles.floatingPayBtn, pressed && { transform: [{ scale: 0.96 }] }]}>
            <LinearGradient colors={gradientColors} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.floatingInner}>
              <Ionicons name="arrow-up-circle" size={hp(3.2)} color="#fff" style={{ marginRight: wp(2) }} />
              <Text style={styles.floatingPayText}>Pay</Text>
            </LinearGradient>
          </Pressable>
        </Animated.View>
      ...
      {cartData.length > 0 && showFooterUI && (
        <View style={[styles.footer, { paddingBottom: Math.max(insets.bottom, hp(2)) }]}>
          <Pressable style={{ alignSelf: "flex-end", marginBottom: hp(0.5) }} onPress={() => setShowFooterUI(false)}><Ionicons name="chevron-down-outline" size={hp(2.4)} /></Pressable>

          <View style={styles.footerRow}>
            <View><Text style={styles.deliveryInformationText}>Use Coins</Text><Text style={styles.deliveryPartnerText}>Available: {String(coinBalance)}</Text></View>
            <Switch value={useCoins} onValueChange={setUseCoins} trackColor={{ false: "#ccc", true: theme.PRIMARY_COLOR }} thumbColor="#fff" />
          </View>

          <Pressable style={styles.footerRow} onPress={() => openPaymentModal("deliveryInstruction")}>
            <View style={{ flexDirection: "column" }}>
              <Text style={styles.deliveryInformationText}>Delivery Instruction</Text>
              <Text style={styles.deliveryPartnerText}>Tap to add or edit delivery note</Text>
            </View>
            <Ionicons name="chevron-forward-outline" size={hp(2.5)} />
          </Pressable>
          ...
          <View>
            <Pressable style={styles.footerRow} onPress={() => openPaymentModal("toPay")}>
              <View><Text style={styles.deliveryInformationText}>To Pay</Text><Text style={styles.deliveryPartnerText}>Incl. all taxes and charges</Text></View>
              <Text style={styles.toPayAmount}>₹{finalAmount.toFixed(2)}</Text>
            </Pressable>

            <Pressable style={styles.proceedBtn} onPress={() => openPaymentModal("proceed")}>
              <Text style={styles.proceedBtnText}>Proceed to Payment • ₹{finalAmount.toFixed(2)}</Text>
            </Pressable>
          </View>
        </View>
      )}

      <BottomSheetModal ref={bottomSheetRef} index={0} snapPoints={snapPoints} backdropComponent={(props) => (<BottomSheetBackdrop {...props} disappearsOnIndex={-1} appearsOnIndex={0} pressBehavior="close" />)} enablePanDownToClose handleIndicatorStyle={{ backgroundColor: "#ddd" }}>
        <BottomSheetView style={styles.bottomSheetView}>
          {modalType === "toPay" ? (
            <View>
              <Text style={styles.modalTitle}>Bill Summary</Text>
              ...
              <View style={{ flexDirection: "row", justifyContent: "space-between", marginVertical: 8, borderTopWidth: 1, borderColor: "#eee", paddingTop: 8 }}>
                <Text style={{ fontWeight: "700" }}>Total</Text>
                <Text style={{ fontWeight: "700" }}>₹{finalAmount.toFixed(2)}</Text>
              </View>

              <Pressable style={[styles.proceedBtn, { marginTop: hp(2) }]} onPress={() => { setModalType("proceed"); }}>
                <Text style={styles.proceedBtnText}>Proceed • ₹{finalAmount.toFixed(2)}</Text>
              </Pressable>

              <Pressable onPress={() => bottomSheetRef.current?.dismiss?.()}><Text style={styles.cancelText}>Cancel</Text></Pressable>
            </View>
          ) : modalType === "deliveryInstruction" ? (
            <View>
              <Text style={styles.modalTitle}>Delivery Instruction</Text>
              <Text style={{ marginBottom: hp(1) }}>Add details for the delivery partner (house hint, gate code, preferred drop spot).</Text>
              <TextInput placeholder="e.g. Leave at the door, call on arrival..." value={deliveryNoteLocal} onChangeText={setDeliveryNoteLocal} multiline style={{ borderWidth: 1, borderColor: "#ddd", borderRadius: 8, minHeight: hp(16), padding: wp(3), textAlignVertical: "top" }} />
              <View style={{ flexDirection: "row", justifyContent: "flex-end", gap: wp(3), marginTop: hp(2) } as any}>
                <Pressable style={[styles.modalBtn, { backgroundColor: "#ccc" }]} onPress={() => bottomSheetRef.current?.dismiss?.()}><Text>Cancel</Text></Pressable>
                <Pressable style={[styles.modalBtn, { backgroundColor: "#562E19" }]} onPress={handleSaveDeliveryNote}><Text style={{ color: "#fff" }}>Save</Text></Pressable>
              </View>
            </View>
          ) : (
            <View>
              ...
              <Pressable style={styles.paymentOption} onPress={() => showToast("Online payment integration not implemented yet.")}>
                <Ionicons name="card-outline" size={24} color={theme.PRIMARY_COLOR} />
                <Text style={styles.paymentText}>Pay Online</Text>
              </Pressable>

              <Pressable style={[styles.paymentOption, !isSubscribed && { opacity: 0.45 }]} onPress={() => { if (!isSubscribed) { showToast("Pay Later requires active subscription"); return; } handlePayLater(); }} disabled={!isSubscribed}>
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: "#fff" },
  emptyCart: { flex: 1, justifyContent: "center", alignItems: "center", paddingHorizontal: wp(6), paddingVertical: hp(4) },
  emptyTextWrapper: { maxWidth: wp(80), alignItems: "center", marginBottom: hp(2), flexWrap: "wrap" },
  emptyTitle: { fontSize: hp(2.2), fontWeight: "600", color: "#333", textAlign: "center", lineHeight: hp(3) },
  button: { backgroundColor: theme.PRIMARY_COLOR, paddingVertical: hp(2), paddingHorizontal: hp(5.1), borderRadius: 10, elevation: 3, marginTop: hp(2) },
  buttonText: { color: "#fff", fontSize: hp(1.9), fontWeight: "700" },
  addressRow: { flexDirection: "row", justifyContent: "space-between", marginHorizontal: wp(4), marginTop: hp(2) },
  addressSub: { fontSize: hp(1.4), color: "#5B5B5B" },
  changeBtn: { borderWidth: 1, borderColor: "#ECECEC", borderRadius: 6, paddingHorizontal: wp(3), justifyContent: "center" },
  changeBtnText: { fontSize: hp(1.4), color: "#5B5B5B" },
  itemsHeader: { flexDirection: "row", justifyContent: "space-between", marginHorizontal: wp(4), marginTop: hp(2) },
  deliveryInfo: { flexDirection: "row", alignItems: "center" },
  deliveryText: { color: theme.PRIMARY_COLOR, marginLeft: 5 },
  itemRow: { flexDirection: "row", alignItems: "center", paddingVertical: hp(1.5), borderBottomWidth: 1, borderColor: "#E8E8E8", marginHorizontal: wp(4), minHeight: hp(12) },
  itemImage: { height: hp(10), width: wp(20), borderRadius: 8 },
  itemBody: { flex: 1, marginLeft: wp(3) },
  itemName: { fontSize: hp(1.8), fontWeight: "500" },
  itemPriceEach: { fontSize: hp(1.4), color: "#5B5B5B" },
  qtyContainer: { flexDirection: "row", alignItems: "center", marginHorizontal: wp(1), justifyContent: "center" },
  qtyButton: { borderWidth: 1, borderColor: "#E8E8E8", paddingVertical: hp(0.5), paddingHorizontal: wp(1.5), borderRadius: 4 },
  qtyValueBox: { minWidth: wp(10), alignItems: "center", justifyContent: "center" },
  qtyText: { marginHorizontal: wp(2) },
  itemTotal: { fontWeight: "bold", minWidth: wp(12), textAlign: "right", marginLeft: wp(2) },
  coinIcon: { position: "absolute", right: wp(2), bottom: hp(1) },
  footer: { borderTopWidth: 1, borderColor: "#E8E8E8", padding: wp(4), backgroundColor: "#fff", position: "absolute", left: 0, right: 0, bottom: 0 },
  footerRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: hp(2) },
  proceedBtn: { backgroundColor: theme.PRIMARY_COLOR, paddingVertical: hp(1.5), borderRadius: 8, alignItems: "center", marginBottom: hp(7) },
  proceedBtnText: { color: "#fff", fontWeight: "bold", fontSize: hp(1.8), textAlign: "center" },
  toPayAmount: { fontWeight: "bold" },
  modalTitle: { fontSize: hp(2.2), fontWeight: "bold", marginBottom: hp(1) },
  paymentOption: { flexDirection: "row", alignItems: "center", paddingVertical: hp(1.5), borderBottomWidth: 1, borderColor: "#E8E8E8", gap: wp(3) },
  paymentText: { fontSize: hp(1.8), marginLeft: wp(3) },
  cancelText: { color: "red", textAlign: "center", marginTop: hp(2), fontWeight: "500" },
  deliveryPartnerText: { color: "#AAAAAA", fontSize: hp(1), paddingLeft: wp(0.2) },
  deliveryInformationText: { color: "black", fontSize: hp(1.6), fontWeight: "500" },
  floatingWrapper: { position: "absolute", bottom: hp(3.5), alignSelf: "center", alignItems: "center", justifyContent: "center" },
  floatingPayBtn: { borderRadius: 999, overflow: "visible", paddingBottom: hp(7) },
  floatingInner: { flexDirection: "row", alignItems: "center", justifyContent: "center", paddingHorizontal: wp(7), paddingVertical: hp(2), borderRadius: 10, shadowColor: theme.SECONDARY_COLOR, shadowOffset: { width: 0, height: 10 }, shadowOpacity: 0.18, shadowRadius: 12, elevation: 8 },
  floatingPayText: { color: "#fff", fontWeight: "700", fontSize: hp(1.8) },
  bottomSheetView: { paddingHorizontal: wp(4), paddingTop: hp(1.2), paddingBottom: hp(7) },
  modalBtn: { paddingVertical: hp(0.8), paddingHorizontal: wp(3), borderRadius: 6, alignItems: "center", justifyContent: "center" }
});

// --------------- RESPONSIVE NOTES ---------------
// 1) You already use hp/wp. To support tablets, consider:
//    - increasing ITEM_ROW_MIN_HEIGHT when WINDOW.width > 900 (or use Platform.isPad)
//    - enlarging fonts: use hp(2.6) for headings on large screens
// 2) Test on landscape: FlatList contentContainerStyle paddingBottom ensures footer doesn't cover items.
// 3) Images: itemImage uses wp(20) (20% width). For very small phones you may want wp(30) fallback — adjust with Dimensions.
// 4) SafeAreaView + insets are already used which helps with notches.
// 5) If you want pixel-perfect responsive: create a single place (responsive.ts) mapping breakpoints to token sizes and import it.
// ------------------------------------------------
